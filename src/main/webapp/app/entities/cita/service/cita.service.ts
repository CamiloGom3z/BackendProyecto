...
type RestOf<T extends ICita | NewCita> = Omit<T, 'fechaCita' | 'fechaFinCita'> & {
...
export class CitaService {
  ...
  protected convertDateFromClient<T extends ICita | NewCita | PartialUpdateCita>(cita: T): RestOf<T> {
    return {
      ...cita,
      fechaCita: cita.fechaCita?.toJSON() ?? null,
      fechaFinCita: cita.fechaFinCita?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCita: RestCita): ICita {
    return {
      ...
      fechaFinCita: restCita.fechaFinCita ? dayjs(restCita.fechaFinCita) : undefined,
    };
  }
