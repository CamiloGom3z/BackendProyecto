...
public class GaleriaImagenServiceImpl implements GaleriaImagenService {
    ...
    public List<GaleriaImagenDTO> findAll() {
        LOG.debug("Request to get all GaleriaImagens");
        return galeriaImagenRepository.findAll().stream().map(galeriaImagenMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
    }
